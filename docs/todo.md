...
- [ ] **Dohledat případné zbylé CS identifikátory**: `grep -rniE 'prechod|navrh|lajn|fotk|vyska|delka|oblast|misto' src/ assets/ templates/` → odfiltrovat UI texty + legacy SQL/sloupce. (Pozn.: dřívější sken ukázal 0 českých `$proměnných`/metod/JS/CSS; většina hitů = legacy SQL + UI text, takže pravděpodobně už nic.)
