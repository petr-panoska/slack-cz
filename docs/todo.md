...
- [ ] **Mobil: přechody na `/mapa`** (2026-07-05) — box Přechody je na mobilu schovaný (`_map.scss`, TODO(mapa-mobile)); vymyslet, jak je zakomponovat do mobilního ovládání (box Lajny už jede jako sbalitelná lišta nahoře, viz redesign 2026-07-07 níž).
- [x] **Redesign boxu Lajny** (2026-07-07) — vlastní BEM blok `.line-feed` (dřív půjčené `.crossing-feed-*` třídy — nedělat!), flat & density: hlavička (celá = toggle) se šipkou na konci, počet lajn bold, řádky bez karet dělené hairlines, tělo bez paddingu. **Výška těla je PEVNÁ, user-driven** (default ~5 řádků, tažením kulatého úchytu na spodní hraně / šipky ↑↓; sessionStorage, clamp 60 px – ½ viewportu vč. hlavičky) — fit-to-content byl zavržen: výška z obsahu → resize mapy → jiné lajny ve výřezu → jiný obsah = nekonečná smyčka. Gradient „dole je toho víc" (`has-overflow`, mizí po doscrollování), sbalení animované přes výšku (jen `height`, ne padding — rozbíjel to na dva kroky; ne `display:none` — zabije transition), `.map-side` z-index 1100 > leaflet 1000 kvůli přesahu úchytu přes mapu. Zároveň `/mapa` bez patičky (blok `footer` v base) a bez svislého scrollu (`100dvh` + `overflow: clip` na wrapperu — schovaný time-travel panel jinak přetékal).
