...
- [x] **Redirect po registraci → rovnou na `app_profile_edit`** (dřív `app_profile`), flash zpráva přepsaná z anglického Symfony-scaffold textu na český: „Děkujeme za registraci! Jestli chceš, můžeš o sobě uvést víc informací. Ať se daří 🍀". Email-verifikační flow (`verifyUserEmail()`) záměrně beze změny (mimo scope).
...
- [x] **Info box v `/profil/uprava`** — Bootstrap `alert-info` + inline SVG ikonka (stejný vzor jako ostatní ručně kreslené ikony v appce), text „Vyplň dle libosti. Všechny údaje jsou nepovinné."
