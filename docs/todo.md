...
- [ ] Zkontrolovat `HighlineController::PRODUCTION_URL` (`https://www.slack.cz`) — používá ji veřejná „Data report" stránka (`/data-report`) pro proklik na legacy detail; upravit, pokud se prod host změní.
...
- [x] **Galerie sociální vrstva + AJAX likes** (2026-05-11) — `HighlinePhotoLike` (UNIQUE photo+user) + `HighlinePhotoComment` (flat, plain-text), detail page `/highline/{slug}/fotky/{id}`, like-toggle bez reloadu (`photo_like_controller.js`, JSON endpoint), homepage „Z galerie" rotace. Cover zatím čistě legacy URL. Migrace `Version20260511134125`.
- [x] **Audit historie + stack-pop curation + verifikační merge** (2026-05-10) — `/highline/{slug}/historie` nad `HighlineEdit` rows, vlastní `beforeSnapshot` per row, admin „Smazat poslední revizi", verify slévá historii do jediné APPLIED creation revize. Maintenance `app:edit:sync-from-history`. Detaily v `docs/highline-edits.md`.
...
- [x] **Parking GPS import** (2026-05-10) — `Highline.parkingLatitude/Longitude`, LEFT JOIN `gps WHERE type='PARKING'` (133/254 lajn), detail + form toggle. Migrace `Version20260510175252`.
- [x] **Highline CRUD + verifikační/proposal flow** (2026-05-10) — `HighlineCrudController`, `Highline.isVerified` + `createdBy`, `HighlineEdit` unified audit log + pending queue, 254 legacy lajn `verified=true`, `ROLE_ADMIN` + `app:admin:grant`, `highline_form_map_controller` 2-endpoint GPS picker. Detaily v `docs/highline-edits.md`.
- [x] **Crossing form (CRUD pro přechody)** — `CrossingController` + `HighlineCrossingForm`, edit/delete vlastních přechodů z deníku.
- [x] **Deník uživatele `/denik/{id}`** — hlavička, mini-mapa navštívených highlines, tabulka přechodů. Linkováno z „Posledních přechodů" + z popupu na `/mapa`.
...
- [x] **Single source of truth pro recent crossings** — `HighlineCrossingRepository::RECENT_LIMIT`, `findRecent()` + `findRecentForJson()`.
...
- [x] **Highline detail `/highline/{slug}`** — slug v DB, stat panel, info tabulka, mini-mapa s polyline mezi body, seznam přechodů.
- [x] **Highline `point1`/`point2` GPS** — naimportováno z legacy `gps` přes JOIN na `point1_id`/`point2_id` (`LINE_POINT`).
...
- [x] Highline import 254/254 z legacy DB do Postgres.
