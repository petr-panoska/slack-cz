...
**Stav k 2026-07-20:** `composer.json` drží všechny Symfony komponenty na
řadě **7.4 LTS**. PHP: **prod 8.3** (Hetzner, native), **dev + CI 8.4**.
...
- [x] **Console commandy** (`app:user:*`, `app:import:*`, `app:admin:grant`,
      `app:edit:sync-from-history`) používají `#[AsCommand]` atribut.
- [x] **Vich uploader mapping** v `LinePhoto` používá PHP atributy
      `#[Vich\Uploadable]` a `#[Vich\UploadableField(...)]`.
...
jako detektor. Plus ručně přes `bin/console debug:container --deprecations`
