...
- Stimulus controllery v `assets/controllers/`:
  ...
  - `map_panel_controller.js` — **tabový panel `.map-panel` na `/mapa`** (Lajny/Přechody, desktop i mobil stejně): přepínání tabů a sbalování — taby jsou Bootstrap buttony (`btn-outline-primary btn-sm`), `.active` (filled) = vybraný tab při rozbaleném panelu; sbalený = jen outline button Lajny + šipka (tab Přechody schová `--crossings` modifikátor), **pevná user-driven výška** tažením úchytu na spodní hraně (clamp: spodní hrana ≤ ½ mapové plochy; výška NIKDY z obsahu — smyčka výška→resize mapy→jiný výřez→jiný obsah) a gradient „dole je toho víc". Obsah panes renderují `line-feed`/`crossing-feed` controllery na **témže elementu**; s panelem mluví Stimulus dispatchi (`line-feed:collapse`, `*:rendered`) přes `data-action` na rootu.
  - `crossing_feed_controller.js` — tab Přechody v `.map-panel`: list posledních N přechodů, filtr (posledních N / rozsah dat) a oko (= visibility emoji markerů na mapě, default schované), reaguje na time-travel režim (změní se v okno -7 dní zpět od virtuálního času). Collapse vlastní panel, ne tenhle controller.
  ...
  - `line_feed_controller.js` — tab Lajny v `.map-panel` (seznam lajn ve výřezu): čistý view, poslouchá `slack:viewport-lines` broadcast z `map_controller` (viz event bus níž), klik na řádek → `slack:line-focus` (+ na mobilu dispatch `collapse`, ať panel nezakrývá popup).
...
Vedle document-eventů běží **element-level Stimulus dispatche** mezi controllery sdílejícími root `.map-panel` (`line-feed:collapse`, `line-feed:rendered`, `crossing-feed:rendered` → `map-panel`), zapojené přes `data-action` na rootu v `map.html.twig`.
