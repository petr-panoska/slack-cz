...
- Stimulus controllery v `assets/controllers/`:
  ...
  - `hp_map_controller.js` (identifier `hp-map`) — homepage mapa v panelu Mapa: ukazuje jen aktivní přechod, vykreslí reálnou linku + emoji walker animaci po lajně a auto-showcase cyklus přes posledních N přechodů. Emoji uživatele dostává ze serverových dat; kanonická paleta je `App\UserEmoji`. Detailní chování v § *Hotové features → Index page*.
...
Emoji paleta účtů má jediný zdroj v `App\UserEmoji::VALUES`. Používá ji
registrační i profilový formulář; nové registraci se předvybere náhodná hodnota
a zvolená emoji se ukládá na `User`. Mapové endpointy ji posílají spolu s
přechodem, takže hlavní i homepage mapa zobrazují stejnou uloženou hodnotu.
`emoji_picker_controller.js` řeší pouze živý náhled ve formuláři.
...
- `symfonycasts/verify-email-bundle` zůstává zapojený, ale odeslání ověřovacího e-mailu je v registraci dočasně vypnuté; nový účet se po uložení rovnou přihlásí
...
### Registrace — 2-krokový flow (aktualizováno 2026-07-20)
...
- **`RegistrationForm`** (`/registrace`): `email`, **`nick`** (`NotBlank` + `Length(max:30)`, `UniqueEntity` na `User` hlídá kolizi), emoji z `App\UserEmoji`, `password` (`NewPasswordType`) a `agreeTerms`. Heslo má živý indikátor síly (`password_strength_controller.js`). Email a nick se při psaní kontrolují POSTem na `/registrace/dostupnost`; endpoint vrací `Cache-Control: no-store`, validuje vstup a má compound rate limiter (30/min, 200/h na IP).
...
- **Po úspěšné registraci** — zápis má vlastní limiter 5 pokusů/h na IP, účet se rovnou přihlásí, zobrazí se česká success flash a následuje redirect na `app_profile_edit`. Odeslání ověřovacího e-mailu je v controlleru zakomentované do rozhodnutí o finálním mail flow; route pro ověření zůstává zachovaná.
