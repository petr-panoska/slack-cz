...
#[AsCommand(
    name: 'app:import:lines',
    description: 'Imports lines from the legacy MySQL database into the new Postgres schema',
)]
final class ImportLinesCommand extends Command
{
    public const LINE_HEIGHT_THRESHOLD = 10;

    public function __construct(
        #[Autowire(service: 'doctrine.dbal.old_connection')]
        private readonly Connection $oldConnection,
        private readonly LineRepository $repository,
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if ($input->getOption('truncate') && !$dryRun) {
            ...
            $this->em->createQuery('DELETE FROM ' . Line::class)->execute();
        }
        ...
        foreach ($rows as $i => $row) {
            ...
            $h->setType($height >= self::LINE_HEIGHT_THRESHOLD ? LineType::Highline : LineType::fromLegacyId((int) $row['typ']));
