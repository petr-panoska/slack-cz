 ...
 *     (NOT recorded in any DB table). Imported as a normal HighlinePhoto and
 *     referenced via Highline.coverPhoto.
 *
 ...
 */
#[AsCommand(
    name: 'app:import:highline-photos',
    description: 'Imports legacy highline cover + gallery photos (files + highline_foto) into the new schema',
)]
final class ImportHighlinePhotosCommand extends Command
{
    ...
    public function __construct(
        ...
        private readonly HighlinePhotoRepository $photos,
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $galleryByHighline = [];
        foreach ($rows as $r) {
            $galleryByHighline[(int) $r['highline_id']][] = $r;
        }
        $io->info(sprintf('Legacy gallery rows: %d (across %d highlines)', count($rows), count($galleryByHighline)));
        ...
        ksort($byLegacyId);
        foreach ($byLegacyId as $legacyId => $highline) {
            $dir = $photosDir . '/' . $legacyId;
            $galDir = $dir . '/foto';
            $galleryRows = $galleryByHighline[$legacyId] ?? [];
            ...
            $firstAscent = $highline->getFirstAscentDate();
            $seen = [];               // md5 => HighlinePhoto  (deduped within this line)
            ...
            foreach (array_values($galleryRows) as $i => $row) {
                ...
                $createdAt = $firstAscent !== null
                    ...
                    : null;
                $photo = $this->makePhoto($highline, (int) $row['id'], $this->cleanCaption($row['text']), $createdAt);
                if (!$dryRun) {
            ...
            if ($hasCover) {
                if ($highline->getCoverPhoto() !== null) {
                    ...
                    if (isset($seen[$md5])) {
                        ...
                        if (!$dryRun) {
                            $highline->setCoverPhoto($seen[$md5]);
                            $this->em->flush();
                        }
                        ...
                        $createdAt = $firstAscent !== null
                            ...
                            : null;
                        $photo = $this->makePhoto($highline, null, null, $createdAt);
                        if (!$dryRun) {
                            $this->attachAndFlush($photo, $coverPath);
                            $highline->setCoverPhoto($photo);
                            $this->em->flush();
                        }
                        $seen[$md5] = $photo;
                        $rep['cover_imported']++;
    ...
    private function truncate(SymfonyStyle $io): void
    {
        ...
        $qb = $this->em->getRepository(HighlinePhoto::class)->createQueryBuilder('p')
    ...
    private function makePhoto(Highline $highline, ?int $legacyId, ?string $caption, ?\DateTimeImmutable $createdAt): HighlinePhoto
    {
     ...
     */
    private function attachAndFlush(HighlinePhoto $photo, string $sourcePath): void
    {
    ...
    private function printReport(
        ...
        $io->listing([
            ...
            sprintf('Highlines with no photos: %d', count($noPhotos)),
        ]);
